...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        parser.add_argument(
            "--load_wiki", action="store_true", help="Load Wikipedia data for entities"
        )
        parser.add_argument(
            "--mark_occurences", action="store_true", help="Mark occurrences in text"
        )
    ...
    def handle(self, *args, **options):
        ...
        if load_wiki:
            persons_with_wiki = Entity.objects.filter(type=Entity.PERSON).exclude(
                wiki_id=""
            )
            self.stdout.write(
                self.style.SUCCESS(
                    f"Loading Wikipedia data for {len(persons_with_wiki)} Person entities..."
                )
            )
            ...
            for entity in persons_with_wiki:
                self.stdout.write(
                    self.style.NOTICE(
                        f"Getting data for entity {entity.lemma} with Wiki ID {entity.wiki_id}"
                    )
                )
            ...
            self.stdout.write(
                self.style.SUCCESS("Updated Wikipedia data for entities.")
            )
        ...
        if mark_occurences:
            ...
            for poem in poems:
                ...
                try:
                    ...
                    self.stderr.write(
                        self.style.ERROR(f"Error parsing XML for poem {poem.id}")
                    )
                    continue
                ...
                for occurrence in occurrences:
                    if occurrence.word_id in words:
                        ...
                        for _ in range(occurrence.length - 1):
                            ...
                            next_word_id_parts[-1] = str(
                                int(next_word_id_parts[-1]) + 1
                            )
                            ...
                            if next_word_id in words:
                                if occurrence.word_id in words:
                                    ...
                                    words[next_word_id].set("d-f-person", "2")

                # Save updated XML back to the database without the temporary root
                poem_ai_text.text = "".join(
                    ET.tostring(root, encoding="unicode").split("<root>")[1:]
                ).rsplit("</root>", 1)[0]
                poem_ai_text.save()
